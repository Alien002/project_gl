...
void rasterize_triangle(driver_state& state, const data_geometry* in[3])
{
    ...
    for(unsigned a = 0; a < 3; ++a){
        double i =(state.image_width / 2.0) * (*in)[a].gl_Position[0] + ((state.image_width / 2.0) - 0.5);
        double j =(state.image_height / 2.0) * (*in)[a].gl_Position[1] + ((state.image_height / 2.0) - 0.5);
